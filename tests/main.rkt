...
"Initialize test bucket to empty"
...
"Dry run of upload"
(s3-sync dir bucket #f #:dry-run? #t #:log count-planned)                
(check-equal? plan-count 3)
(check-equal? (ls bucket/) '())

"Upload three files"
(s3-sync dir bucket #f)
(check-equal? (ls bucket/) '("x_test" "y_test" "z_test"))

"Check upload needs no files"
(set! plan-count 0)
(s3-sync dir bucket #f #:log count-planned)
(check-equal? plan-count 0)

"Check download needs no files"
(s3-sync dir bucket #f #:upload? #f #:delete? #t #:log count-planned)
(check-equal? plan-count 0)
...
"Download, not getting newly nested path"
(set! plan-count 0)
(s3-sync dir bucket #f #:shallow? #t #:upload? #f #:log count-planned)
(check-equal? plan-count 0)
(check-equal? (file-list)
              '("sub/a_test" "sub/b_test" "x_test" "y_test" "z_test"))

"Download to get newly nested path"
(s3-sync dir bucket #f #:upload? #f #:log count-planned)
(check-equal? plan-count 2)
(check-equal? (file-list)
...
"Upload to add subpath"
(check-equal? (ls bucket/)
              '("nested/a_test" "nested/b_test" "x_test" "y_test" "z_test"))
(set! plan-count 0)
(s3-sync dir bucket #f #:shallow? #t #:log count-planned)
(check-equal? plan-count 2)
(check-equal? (ls bucket/)
              '("nested/a_test" "nested/b_test" "sub/a_test" "sub/b_test" "x_test" "y_test" "z_test"))
